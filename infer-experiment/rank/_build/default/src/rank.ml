...
module Make (FeatVec : Feature.FEAT_VEC) = struct
  ...
  module WeightedAlarm = struct
    ...
    let list_of_weighted_traces n wtraces = 
      print_endline "weighted_trace map start";
      let result = List.filteri (fun i _ -> i < n) wtraces |> List.map WeightedTrace.to_json  in
      print_endline "weighted_trace map end";
      result

    let list_of_n_weighted_traces n wtraces =
      list_of_weighted_traces n wtraces
  ...
  let report_json map =
    SigRankMap.iter
      (fun program wreport ->
        ...
        let oc = open_out oname in
        print_endline "processing start";
        let a = `List (List.map WeightedAlarm.to_json wreport) in
        print_endline "processing a";
        let b = Yojson.Safe.to_string a in
        print_endline "processing b";
        let c = Yojson.Safe.prettify b in
        print_endline "processing c";
        Printf.fprintf oc "%s" c;
        close_out oc)
      map

  let report map =
    print_endline "report1";
    report_text map;
    print_endline "report2";
    report_json map
